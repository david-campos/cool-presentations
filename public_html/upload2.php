...
echo $id_code.','.$name.','.$start.','.$fin.','.$lat.','.$lon.','.$access_code.','.$user_id;
prueba($mysqli,$id_code,$name,$start,$fin,$lat,$lon,$access_code,$user_id);


function prueba($mysqli,$id_code,$name,$start,$fin,$lat,$lon,$access_code,$user_id){
	$stmt = $mysqli->prepare('INSERT INTO presentations VALUES (?,?,?,?,?,?,?,?)');
	$stmt->bind_param('ssssddsi', $id_code,$name,$start,$fin,$lat,$lon,$access_code,$user_id);
	if(!$stmt->execute()) {
		http_response_code(500);
        $stmt->close();
        $mysqli->close();
